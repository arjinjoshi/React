...
import { removeFromWatchHistoryCollection } from '../redux/slices/WatchHistorySlice';
import { removeFromWatchListCollection } from '../redux/slices/WatchListSlice';

export type CurrentTab = "watchlist" | "watchhistory";
interface CollectionCardProps {
    item: Search;
    currentTab: string;
}

const CollectionCard = ({item, currentTab}: CollectionCardProps) => {
  ...
  return (
    <div className="w-1/6 h-110 rounded-xl z-50 border-none relative group overflow-hidden transition-all duration-300 hover:scale-105">
      ...
      {currentTab === "watchlist" ?
        <button onClick={ () =>  {
              dispatch(removeFromWatchListCollection(item.id));
            }} className='bg-(--var4) active:scale-95 text-white rounded px-3 py-1 font-medium '>
              - WatchList
        </button>
      :
        <button onClick={ () => {
              dispatch(removeFromWatchHistoryCollection(item.id));
            }} className='bg-(--var4) active:scale-95 text-white rounded px-3 py-1 font-medium '>
              - Watched
        </button>
      }
